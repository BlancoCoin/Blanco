...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBlancoAmount"))
        settings.setValue("nAnonymizeBlancoAmount", 1000);
    nAnonymizeBlancoAmount = settings.value("nAnonymizeBlancoAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBlancoAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBlancoAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBlancoAmount:
            return QVariant(nAnonymizeBlancoAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBlancoAmount:
            nAnonymizeBlancoAmount = value.toInt();
            settings.setValue("nAnonymizeBlancoAmount", nAnonymizeBlancoAmount);
            emit AnonymizeBlancoAmountChanged(nAnonymizeBlancoAmount);
            break;
        default:
            break;
        }
