...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBlancoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBlancoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBlancoAmount = strAnonymizeBlancoAmount.remove(strAnonymizeBlancoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBlancoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBlancoAmount*COIN) nMaxToAnonymize = nAnonymizeBlancoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBlancoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBlancoAmount));
        strAnonymizeBlancoAmount = strAnonymizeBlancoAmount.remove(strAnonymizeBlancoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBlancoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
