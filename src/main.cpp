...
const string strMessageMagic = "Blanco Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
	int64_t ret;
	if (nHeight < 2501) {
		ret = blockValue * 2 / 3;  //66.66%
	}
	else if (nHeight < 3001) {
		ret = blockValue * 4 / 5;  //80%
	}
	else if (nHeight < 4001) {
		ret = blockValue * 8 / 9;  //88.88%
	}
	else if (nHeight < 5501) {
		ret = blockValue * 4 / 5;  //80%
	}
	else if (nHeight < 6501) {
		ret = blockValue * 5 / 6;   //83.33%
	}
	else {
		ret = blockValue * 6 / 7;   //85.71%
	}
