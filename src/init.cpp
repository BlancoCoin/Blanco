...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Blanco.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Blancod.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeBlancoamount=<n> " + _("Keep N Blanco anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Blanco is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Blanco is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Blanco version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Blanco server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Blanco") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Blanco to complete") << "\n";
    ...
    nAnonymizeBlancoAmount = GetArg("-anonymizeBlancoamount", 0);
    if(nAnonymizeBlancoAmount > 999999) nAnonymizeBlancoAmount = 999999;
    if(nAnonymizeBlancoAmount < 2) nAnonymizeBlancoAmount = 2;
    ...
    LogPrintf("Anonymize Blanco Amount %d\n", nAnonymizeBlancoAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1BAC+1000 == (.1BAC+100)*10
       10BAC+10000 == (1BAC+1000)*10
    */
