...
int nAnonymizeBlancoAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Blanco
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Blanco
    // Mac: ~/Library/Application Support/Blanco
    // Unix: ~/.Blanco
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Blanco";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Blanco.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Blancod.pid"));
