...
//
// Bootup the masternode, look for a 5000 BAC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in Blanco.conf */
...
{
    CBlancocoinAddress address(collateralAddress);
